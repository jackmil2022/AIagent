...
⑤ **硬限制 vs 软限制**：
- 硬限制：$|w| \leq \theta$，参数被约束在一个边界内，边界上可能出现不连续的跳变
- 软限制（L2 正则）：$L = L_0 + \frac{\lambda}{2}\|w\|^2$，通过惩罚项平滑地鼓励权重变小，$\lambda$ 控制惩罚力度

⑥ **为什么用 L2 而不是 L1？** L2 正则（权重衰退）使权重趋向于小的非零值（权重分散），而 L1 正则使权重趋向于稀疏（部分权重变为 0）。L2 在深度学习中更常用，因为它更容易优化。

⑦ **λ 的选择**：$\lambda$ 越大，正则化越强，模型越简单。需要通过验证集来选择合适的 $\lambda$。
...
⑤ **L2 正则项惩罚大权重的原因**：
- 大权重意味着模型对输入的微小变化非常敏感，容易过拟合噪声
- 大权重导致模型函数变化剧烈，泛化能力差
- 通过惩罚大权重，鼓励模型使用更多特征的组合，而不是依赖少数特征
