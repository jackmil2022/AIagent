...
本书使用 **PyTorch** 作为实践的基础框架。PyTorch 由 Facebook（现Meta）于 2016 年开源，是目前学术界和工业界使用最广泛的深度学习框架之一，凭借动态计算图、简洁的 Python API 和庞大的生态系统，已经成为深度学习研究和应用的事实标准。
...
除 PyTorch、torchvision 外，本书还会用到 `numpy`、`matplotlib`（全书绘图与数值计算）、`scikit-learn`（第 2、3 章加载加州房价、鸢尾花数据集）、`networkx` 与 `torch_geometric`（第 9 章图神经网络）。这些依赖都列在配套仓库的 `requirements.txt` 中。

两条统一约定：

- **以 PyTorch 2.x 为基准**：代码采用 PyTorch 2.x 官方推荐写法。少数 API 对版本敏感，例如 `torch.load(..., weights_only=True)` 需 PyTorch 2.4+，张量范数统一用 `torch.linalg.norm`。遇到旧资料里已弃用的写法，以本仓库代码为准。
- **固定随机种子**：凡是会产生数值结果的实验，都会在数据生成与建模前调用 `torch.manual_seed` 固定随机性。notebook 中的输出、曲线为对应随机种子下的典型结果；更换随机种子、硬件或库版本时数值可能略有出入，但结论方向不变。
...
张量支持基础数学运算、逻辑运算、矩阵运算等百余种运算。以加法为例，调用方式可归为两类（共三种具体写法）：
...
torch.linalg.norm(x, ord='fro')  # 矩阵的弗罗贝尼乌斯范数
torch.linalg.vector_norm(x - y, ord=2)  # 向量 (x-y) 的 2-范数
...
print('Frobenius   :', torch.linalg.norm(A, ord='fro').item())
```
...
- **加州房价数据集**：20,640 条样本，每条记录给出街区层面的 8 个特征（中位收入、房龄、平均房间数等）和房价中位数。
