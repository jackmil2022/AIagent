...
本章引入的 `RunnerV1` 只覆盖闭式解场景；第 3、4 章会循序渐进扩展到 `RunnerV2` / `RunnerV3`。三版本的完整实现都打包在 `90-资料库/02-课程原始资料/03-深度学习/pytorch/nndl/runner.py` 里，后续章节可直接 `from nndl.runner import RunnerV1/V2/V3` 复用。
...
```bash
python -m pytest 90-资料库/02-课程原始资料/03-深度学习/pytorch/tests/test_chap2.py -v
```
