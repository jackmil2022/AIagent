...
- `Latitude` 权重为负——纬度越高（越往北）预测房价越低；这只是该线性模型的粗略倾向，并非严格的地理事实（加州房价最高的旧金山湾区其实在北部沿海）；
- `AveRooms` 权重为负——在已包含 `AveBedrms`、`AveOccup` 等共线特征的线性模型里，平均房间数的边际贡献被压成了负值（与正文输出的 `AveRooms -0.2692` 一致）。
