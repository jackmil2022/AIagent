...
# label[0, 1]=1.; label[1, 0]=1; label[1, 1]=1
# label[2, 3]=1; label[3, 5]=1; label[4, 0]=1
# aa = Softmax()
# out = aa.forward(x)
...
# y = torch.softmax(x_t, dim=-1)
# (y * label_t).sum().backward()
# print(x_t.grad)
...
label_t = torch.tensor(label)
h1 = torch.matmul(x_t, W1_t)
h1_relu = torch.relu(h1)
h2 = torch.matmul(h1_relu, W2_t)
prob = torch.softmax(h2, dim=-1)
log_prob = torch.log(prob)
loss = (label_t * log_prob).sum()
