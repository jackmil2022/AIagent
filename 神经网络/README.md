...
```bash
# 在仓库根目录
jupyter notebook chap5卷积神经网络/卷积神经网络-上.ipynb
```
...
[`nndl/`](nndl/) 是随书工程化版本的算子 / 模型 / 训练框架，被多个章节复用。各章 notebook 在**首次引入**处会内联展示完整实现帮助讲解；后续章节 / 测试通过 `from nndl import ...` 直接调用工程化版本。
