...
② **验证集（Validation Set）**：用于调整超参数（如学习率、网络层数等），选择最优模型。

③ **测试集（Test Set）**：最终评估模型泛化能力的数据，只能用一次。

④ 为什么要分三个集合？
- 如果用训练集评估模型，会高估模型性能（过拟合）
- 如果用测试集调参，测试集的信息会泄露到模型中，评估结果不可靠
...
⑥ **交叉验证（Cross-Validation）**：将数据分成 $k$ 份，每次用 $k-1$ 份训练、1 份验证，重复 $k$ 次取平均。适合数据量较小时使用。
...
① **欠拟合（Underfitting）**：模型在训练集和测试集上的表现都很差，说明模型太简单，没有学到数据的规律。

② **过拟合（Overfitting）**：模型在训练集上表现很好，但在测试集上表现很差，说明模型"死记硬背"了训练数据，泛化能力差。

③ **模型容量（Model Capacity）**：模型拟合各种函数的能力。容量越高，能学到的函数越复杂。
- 低容量模型：容易欠拟合
- 高容量模型：容易过拟合

④ **偏差-方差权衡（Bias-Variance Tradeoff）**：
- **偏差（Bias）**：模型预测的期望值与真实值的差距，高偏差 = 欠拟合
- **方差（Variance）**：模型在不同训练集上预测结果的波动，高方差 = 过拟合
...
⑥ **解决方法**：
- 欠拟合：增加模型容量（更多层/更多神经元）、减少正则化、增加训练轮数
- 过拟合：增加数据量、数据增广、正则化（权重衰退、Dropout）、提前停止
