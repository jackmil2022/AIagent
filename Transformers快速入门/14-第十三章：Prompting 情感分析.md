...
可以看到，MLM 任务与[序列标注](https://transformers.run/2022/03/18/transformers-note-6.html)任务很相似，也是对 token 进行分类，并且类别是整个词表，不同之处在于 MLM 任务只需要对文中特殊的 $\\texttt{\[MASK\]}$ token 进行标注。因此在处理数据时我们需要：
