...
> 来源：https://transformers.run/c3/2022-03-08-transformers-note-5/
...
在[《开箱即用的 pipelines》](https://transformers.run/2021/12/08/transformers-note-1.html)中我们已经介绍过，序列标注任务在 Transformers 库中被称为 token 分类任务，典型的如命名实体识别 (NER)，负责识别出文本中哪些片段是实体。
...
![IOB\_versions.png](https://transformers.run/assets/img/transformers-note-5/iob_versions.png)
...
![question\_tokens.png](https://transformers.run/assets/img/transformers-note-5/question_tokens.png)
...
\\\[P(\\text{index}\_{start}, \\text{index}\_{end}) = P\_{start}(\\text{index}\_{start})\\times P\_{end}(\\text{index}\_{end})\\\]
