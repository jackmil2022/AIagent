...
![transformers\_chrono](https://transformers.run/assets/img/transformers/transformers_chrono.svg)
...
-   基于句子的前 $n$ 个词来预测下一个词，因为输出依赖于之前和当前的输入，因此该任务被称为**因果语言建模**（Causal Language Modeling，CLM），如图 2-2 所示；
    ...
    ![causal\_modeling](https://transformers.run/assets/img/transformers/causal_modeling.svg)

    图 2-2 因果语言建模示意图

-   基于上下文，即周围的词语，来预测句子中被遮盖掉的词语（Masked Word），因此该任务被称为**遮盖语言建模**（Masked Language Modeling，MLM），如图 2-3 所示。

    ![masked\_modeling](https://transformers.run/assets/img/transformers/masked_modeling.svg)
...
![nlp\_models\_size](https://transformers.run/assets/img/transformers/nlp_models_size.png)

图 2-4 Transformer 模型规模的变化趋势

但是，从头训练一个预训练语言模型，尤其是大型语言模型，需要海量的数据，不仅时间和计算成本非常高，对环境的影响也很大，如图 2-5 所示：

![carbon\_footprint](https://transformers.run/assets/img/transformers/carbon_footprint.svg)
...
![pretraining](https://transformers.run/assets/img/transformers/pretraining.svg)
...
![finetuning](https://transformers.run/assets/img/transformers/finetuning.svg)
...
![transformers\_blocks](https://transformers.run/assets/img/transformers/transformers_blocks.svg)
...
![transformers\_architecture](https://transformers.run/assets/img/transformers/transformers_architecture.png)
...
![transformers\_architecture](https://transformers.run/assets/img/transformers/AttentionSceneFinal.gif)
...
![transformers\_architecture](https://transformers.run/assets/img/transformers/token_selection.png)
...
![transformers\_architecture](https://transformers.run/assets/img/transformers/repeat_penalty.png)
...
![transformers\_architecture](https://transformers.run/assets/img/transformers/beam_search.png)
